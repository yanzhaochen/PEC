    ...
    for k, sents in tqdm(data.items()):
        if k in all_speakers:
            ...
            for sent in sents[:num_personas]:
                tokenized_words.extend(
                    tokenizer.tokenize(" ".join(sent))[:22] + ["[SEP]"]
                )
...
def convert_conversations_to_ids(
    data, persona, tokenizer, max_seq_len, max_sent_len, num_personas
):
                ...
                for sent_ids in all_persona_sent_ids:
                    ...
                    if sent not in response_sent:
                        filtered_tokens.extend(
                            sent_ids + [tokenizer.convert_tokens_to_ids("[SEP]")]
                        )
    ...
    for context, response, speaker in tqdm(data):
        ...
        )  # convert to token ids, add [cls] and [sep] at beginning and end
        response_ids = tokenizer.encode(response, add_special_tokens=True)
        context_lens.append(len(context_ids))

        context_ids, context_attention_mask, context_token_type_ids = pad_tokens(
            context_ids, max_seq_len, "context"
        )
        response_ids, response_attention_mask, response_token_type_ids = pad_tokens(
            response_ids, max_sent_len + 2, "response"
        )
        ...
        if persona is not None:
            ...
            persona_ids, persona_attention_mask, persona_token_type_ids = pad_tokens(
                persona_ids, max_persona_len, "persona", num_personas, response_ids
            )
    ...
    all_context_attention_mask = torch.tensor(
        all_context_attention_mask, dtype=torch.long
    )
    all_context_token_type_ids = torch.tensor(
        all_context_token_type_ids, dtype=torch.long
    )

    # (num_examples, max_sent_len)
    all_response_ids = torch.tensor(all_response_ids, dtype=torch.long)
    all_response_attention_mask = torch.tensor(
        all_response_attention_mask, dtype=torch.long
    )
    all_response_token_type_ids = torch.tensor(
        all_response_token_type_ids, dtype=torch.long
    )

    if persona is not None:
        # (num_examples, max_persona_len)
        all_persona_ids = torch.tensor(all_persona_ids, dtype=torch.long)
        all_persona_attention_mask = torch.tensor(
            all_persona_attention_mask, dtype=torch.long
        )
        all_persona_token_type_ids = torch.tensor(
            all_persona_token_type_ids, dtype=torch.long
        )

    cprint(
        all_context_ids.shape,
        all_context_attention_mask.shape,
        all_context_token_type_ids.shape,
    )
    cprint(
        all_response_ids.shape,
        all_response_attention_mask.shape,
        all_response_token_type_ids.shape,
    )

    if persona is not None:
        cprint(
            all_persona_ids.shape,
            all_persona_attention_mask.shape,
            all_persona_token_type_ids.shape,
        )
...
def match(model, matching_method, x, y, x_mask, y_mask):
    ...
    attn_mask = torch.bmm(
        x_mask.unsqueeze(-1), y_mask.unsqueeze(1)
    )  # (batch_size, m, n)
    attn = torch.bmm(x, y.transpose(1, 2))  # (batch_size, m, n)
    model.attn = attn
    model.attn_mask = attn_mask

    x_to_y = torch.softmax(
        attn * attn_mask + (-5e4) * (1 - attn_mask), dim=2
    )  # (batch_size, m, n)
    y_to_x = torch.softmax(attn * attn_mask + (-5e4) * (1 - attn_mask), dim=1).transpose(
        1, 2
    )  # # (batch_size, n, m)
    ...
    y_attn = torch.bmm(
        y_to_x.mean(dim=1, keepdim=True), x_to_y
    )  # (batch_size, 1, n) # true important attention over y
    x_attn = torch.bmm(
        x_to_y.mean(dim=1, keepdim=True), y_to_x
    )  # (batch_size, 1, m) # true important attention over x
...
def aggregate(model, aggregation_method, x, x_mask):
    ...
    if aggregation_method == "mean":
        return (x * x_mask.unsqueeze(-1)).sum(dim=1) / x_mask.sum(
            dim=-1, keepdim=True
        ).clamp(
            min=1
        )  # (batch_size, emb_size)
    ...
    if aggregation_method == "max":
        return x.masked_fill(x_mask.unsqueeze(-1) == 0, -5e4).max(dim=1)[
            0
        ]  # (batch_size, emb_size)
    ...
    if aggregation_method == "mean_max":
        return torch.cat(
            [
                (x * x_mask.unsqueeze(-1)).sum(dim=1)
                / x_mask.sum(dim=-1, keepdim=True).clamp(min=1),
                x.masked_fill(x_mask.unsqueeze(-1) == 0, -5e4).max(dim=1)[0],
            ],
            dim=-1,
...
def fuse(
    ...
    batch_x_emb, batch_y_emb_context = match(
    ...
    batch_x_emb = aggregate(
        model, aggregation_method, batch_x_emb[0], batch_x_mask
    )  # batch_x_emb: (batch_size*num_candidates, 2*emb_size)
    batch_y_emb_context = aggregate(
        model, aggregation_method, batch_y_emb_context[0], batch_y_mask
    )  # batch_y_emb_context: (batch_size*num_candidates, 2*emb_size)

    if batch_persona_emb is not None:
        batch_persona_emb, batch_y_emb_persona = match(
            model,
            matching_method,
            batch_persona_emb,
            batch_y_emb,
        ...
        batch_persona_emb = aggregate(
            model, aggregation_method, batch_persona_emb[0], batch_persona_mask
        )  # batch_persona_emb: (batch_size*num_candidates, 2*emb_size)
        batch_y_emb_persona = aggregate(
            model, aggregation_method, batch_y_emb_persona[0], batch_y_mask
        )  # batch_y_emb_persona: (batch_size*num_candidates, 2*emb_size)
        ...
        return torch.bmm(
            ...
            torch.cat(
                [
                    batch_y_emb_context,
                    batch_y_emb_context_2hop,
                    batch_y_emb_persona,
                    batch_y_emb_persona_2hop,
                ],
                dim=-1,
            ).unsqueeze(-1),
        ...
        return torch.bmm(
            torch.cat([batch_x_emb, batch_x_emb_2hop], dim=-1).unsqueeze(1),
            torch.cat([batch_y_emb_context, batch_y_emb_context_2hop], dim=-1).unsqueeze(
                -1
            ),
        ).reshape(batch_size, num_candidates)
...
def dot_product_loss(batch_x_emb, batch_y_emb):
    ...
    if batch_x_emb.dim() == 2:
        ...
        dot_products = torch.bmm(
            batch_x_emb,
            batch_y_emb.unsqueeze(0).repeat(batch_size, 1, 1).transpose(1, 2),
        )[
            :, targets, targets
...
def train_epoch(
    ...
    if len(models) == 1:
        if num_personas == 0:
            ...
            context_model, response_model, persona_model = (
                models[0],
                models[0],
                models[0],
            )
    ...
    for i, batch in enumerate(data_iter):
        ...
        if apply_interaction:
            ...
            if has_persona:
                ...
                batch_persona_emb = output_persona[
                    0
                ]  # (batch_size, persona_len, emb_size)

                batch_persona_emb = batch_persona_emb.repeat_interleave(
                    num_candidates, dim=0
                )
                batch_persona_mask = batch_persona_mask.repeat_interleave(
                    num_candidates, dim=0
                )

            batch_x_emb = batch_x_emb.repeat_interleave(
                num_candidates, dim=0
            )  # (batch_size*num_candidates, context_len, emb_size)
            batch_x_mask = batch_x_mask.repeat_interleave(
                num_candidates, dim=0
            )  # (batch_size*num_candidates, context_len)

            # interaction
            # context-response attention
            batch_y_emb = (
                batch_y_emb.unsqueeze(0)
                .repeat(batch_size, 1, 1, 1)
                .reshape(-1, sent_len, emb_size)
            )  # (batch_size*num_candidates, sent_len, emb_size)
            batch_y_mask = (
                batch_y_mask.unsqueeze(0).repeat(batch_size, 1, 1).reshape(-1, sent_len)
            )  # (batch_size*num_candidates, sent_len)
            logits = fuse(
                context_model,
                matching_method,
                aggregation_method,
        ...
        if apply_interaction:
            ...
            with torch.no_grad():
                for i in range(num_chunks):
                    ...
                    batch_x_emb.append(
                        mini_output_x[0]
                    )  # [(chunk_size, seq_len, emb_size), ...]
                    batch_x_pooled_emb.append(mini_output_x[1])
                batch_x_emb = torch.cat(
                    batch_x_emb, dim=0
            ...
            with torch.no_grad():
                ...
                for i in range(batch_size):
                    x_emb = batch_x_emb[i : i + 1].repeat_interleave(
                        num_candidates, dim=0
                    )  # (num_candidates, context_len, emb_size)
                    x_mask = batch_x_mask[i : i + 1].repeat_interleave(
                        num_candidates, dim=0
                    )  # (batch_size*num_candidates, context_len)
                    persona_emb, persona_mask = None, None
                    if has_persona:
                        persona_emb = batch_persona_emb[i : i + 1].repeat_interleave(
                            num_candidates, dim=0
                        )
                        persona_mask = batch_persona_mask[i : i + 1].repeat_interleave(
                            num_candidates, dim=0
                        )
                ...
                targets = torch.arange(
                    batch_size, dtype=torch.long, device=batch[0].device
                )
                loss = F.cross_entropy(logits, targets)
...
def main(config, progress):
    ...
    if num_personas > 0:
        ...
        if os.path.exists(cached_tokenized_persona_path):
            ...
            all_speakers = set(
                [
                    s
                    for conv in load_pickle(config["train_path"])
                    + load_pickle(config["valid_path"])
                    + load_pickle(config["test_path"])
                    for s, sent in conv
                ]
            )
    ...
    if PEC_ratio != 0:
        ...
        train[:num_PEC_examples] = load_pickle(
            PEC_train_path.replace(".pkl", "_tokenized.pkl")
        )[:num_PEC_examples]
    ...
    if not test_mode:
        train_dataset = convert_conversations_to_ids(
            train, persona, tokenizer, max_seq_len, max_sent_len, num_personas
        )
    ...
    valid_dataset = convert_conversations_to_ids(
        valid, persona, tokenizer, max_seq_len, max_sent_len, num_personas
    )
    valid_sampler = RandomSampler(valid_dataset)
    valid_dataloader = DataLoader(
        valid_dataset, sampler=valid_sampler, batch_size=num_candidates
    )
    ...
    if test_mode:
        ...
        valid_loss, (valid_acc, valid_recall, valid_MRR) = evaluate_epoch(
            valid_iterator,
            models,
            num_personas,
            gradient_accumulation_steps,
        ...
        cprint(
            "test loss: {0:.4f}, test acc: {1:.4f}, test recall: {2}, test MRR: {3:.4f}".format(
                valid_loss, valid_acc, valid_recall, valid_MRR
            )
        )
        sys.exit()
    ...
    for epoch in range(epochs):
        ...
        cprint(
            "Config id: {7}, Epoch {0}: train loss: {1:.4f}, valid loss: {2:.4f}, train_acc: {3:.4f}, valid acc: {4:.4f}, valid recall: {5}, valid_MRR: {6:.4f}".format(
                epoch + 1,
                train_loss,
                valid_loss,
                train_acc,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Model for Transformer-based Dialogue Generation with Controlled Emotion"
    )
    ...
    for k, v in config.items():
        ...
        if isinstance(v, list) and k not in keys_to_omit:
            ...
            and isinstance(config[k][0], list)
        ):
            parameters_to_search[k] = v
        else:
    ...
    if len(parameters_to_search) == 0:
        ...
        results = [
            pool.apply_async(main, args=(x, i / num_configs))
